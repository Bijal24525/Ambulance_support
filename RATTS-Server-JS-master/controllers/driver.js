...
module.exports.get = {
  all: (req, res, next) => {
    Driver.findAll({
      attributes: ['name', 'driver_id', 'email', 'contact', 'location', 'role']
    })
...
module.exports.post = {
  signup: (req, res, next) => {
    const { name, driver_id, email, contact, location, password } = req.body;
  ...
  update: (req, res, next) => {
    ...
    Driver.findOne({
      ...
      .then(driver => {
        ...
        const { name, driver_id, email, contact, location } = req.body;
